...
### ✅ B4（已修復，`8633350`）. `src/components/calendar/CalendarGrid.js:57-198` — 無 `React.memo`，且接收整包 `tasks`（全部月份）
...
**修復狀態**：已補 `React.memo` + `useMemo(getMonthDates)`；同步把父層傳入的 `dropTaskOnDate`（`useTaskMove` hook）與 `getCurrentDate` 改用 `useCallback` 穩定參照（否則 memo 不會生效）。「`tasks` 改傳摘要」與「拆 `CalendarDayCell`」屬長期建議，未做。

### ✅ B5（已修復，`bd2eb18`）. `src/components/calendar/TaskListArea.js` — 排序結果與 `renderItem` 身分不穩定
...
### ✅ C1（已修復，`14eba99`）+ C2（評估後跳過）：`src/locales/index.js:1-12` 全量 eager import 三語系，且 Terms/Privacy 文字佔每個語系檔 61-68%
...
**修復狀態**：C1（Terms/Privacy 抽成 `src/locales/terms/*.js`，`TermsScreen`/`PrivacyScreen` 改用 `useLazyTermsTranslations` 動態 `import()`）已完成，已捕捉大部分體積（61-68%）。C2（`locales/index.js` 改成依 `language` 動態載入單一語系模組）評估後**決定跳過**：C1 完成後剩餘每語系體積已從 21-30KB 降到約 8-10KB，殘餘問題已大幅縮小；而 C2 需要改動 `App.js` 的核心啟動載入狀態機（`showingSplash` 閘門），本專案曾有「啟動時同步載入 CJK 字型導致 App Hanging」的真實事故（見 memory「啟動字型陷阱」），對啟動路徑的變動風險與 C1 完成後的剩餘效益不成比例，故不做。

### C3（已修復，`14eba99`）. 次要清理
- `src/locales/en.js:234-237`、`es.js:237-240`、`zh-Hant.js:233-236` — `bugReport`/`improvementSuggestion`/`featureRequest`/`feedbackOther` 共 12 個 key 全 repo 查無引用（`SupportScreen.js` 實際用的是 `feedbackLove`/`feedbackBug`/`feedbackIdea`/`feedbackQuestion`），確認是改名後留下的舊 key，已刪除。
- `src/utils/dateUtils.js:31-124`（`formatTimestamp`）— 94 行的死程式碼，被 `CalendarScreen.js:40` import 但全 repo 查無任何呼叫點，且與 locale 檔內既有 `months` 陣列邏輯重複，確認後已移除，並清掉 `CalendarScreen.js` 對應的 import。

---

## D. iOS Widget（Swift）重整策略（🟠 中高優先）

### ✅ D1（已修復，`695ab2c`）. `TaskCalWidget.swift:163-164` — Timeline 排程永遠取「明天午夜」與「下一小時」的較小值，等於每小時強制重整
...
**修復狀態**：已覆核確認 RN 端 `widgetService.js` 早已在資料變動時呼叫原生模組 `WidgetReloader.reloadAllWidgets()`/`reloadWidgetWithData()`（內部呼叫 `WidgetCenter.shared.reloadAllTimelines()`），因此拿掉每小時強制重整是安全的——資料變動本來就已經有主動觸發路徑，不會因為這個改動變成要等到隔天才更新。改為 `policy: .after(midnight)`。已用 Xcode 26 對 `TaskCalWidgetExtension` scheme 做 iphonesimulator debug build 確認 `BUILD SUCCEEDED`。

### ✅ D2（已修復，`695ab2c`）. 疊加問題
- **`TaskCalWidget.swift:102-113, 167-170, 183-190`** — `monthName`/`weekdayAbbr`/`todayKey`/`kickerString` 每次呼叫都新建一個 `DateFormatter`（Apple 文件明確指出這是昂貴且應快取重用的物件）。已改為模組層級 `private let` 共用實例（`monthNameFormatter`/`enUSSymbolsFormatter`/`dayKeyFormatter`）。
- **`TaskCalWidget.swift:173`** — `UserDefaults(suiteName:)` 每次呼叫都重新建立實例，`appGroupId` 是常數。已改為模組層級 `private let widgetUserDefaults`。
- **`getSnapshot`/`getTimeline` 各自重新讀取解碼**這一項評估後**跳過**：兩者是 WidgetKit 在不同時機各自呼叫（非同一次刷新內的重複呼叫），且資料量小、解碼成本低；跨呼叫快取需要額外的失效機制，在沒有明確加速需求下引入這層快取的複雜度與潛在過期風險不划算，故保留現況（兩者各自呼叫同一個已存在的共用私有方法 `loadAllTasksByDate()`，程式碼本身沒有重複）。

### D3（評估後跳過）
- **`TaskCalWidget.swift:308-319`** — `body` 對所有 widget family 都無條件計算排序/過濾（`sorted`/`todoCount`/`doneCount`/`nextTime`），但 `largeLayout` 實際上是自己重新排序（line 633），這些預先算好的值對 large family 完全浪費；lock screen family 也用不到。建議把這些衍生值下放到各自 layout 內部按需計算。**跳過原因**：每天的任務數量通常是個位數到十幾筆，`sorted`/`filter`/`count` 在這個規模下的成本可忽略不計；要把這些值下推到各 layout 內部需要重新拆分 `body`/`layoutView` 的函式簽章，屬中度重構，在沒有能實機互動驗證 widget 畫面的環境下，這個改動的風險（可能改錯某個 family 該用哪個衍生值）相對其可感知效益不划算。
- **`TaskCalWidget.swift:630`（`buildCalendarDays`）** — 同一年月的結果固定，但每次 large widget render 都重新計算，建議以「年-月」為 key 做簡易快取。**跳過原因**：純函式、計算量小（35-42 個 struct），改用模組層級可變快取字典會引入 SwiftUI 可能並發 render 多個 widget family 時的共享可變狀態風險，為了這麼小的計算量承擔這個風險不划算。

---

## E. 計時器／動畫生命週期管理（🟡 中優先）

### ✅ E1（已修復，`d93ac94`）. `src/screens/SplashScreen.js:1171-1202`（`checkSessionWithRetry`）— 遞迴 `setTimeout` 重試鏈未在 unmount 時清除
...
**修復狀態**：新增 `sessionRetryTimeoutIdsRef`，每次排程重試都把 timeout id push 進陣列，並新增一個 unmount cleanup effect 統一 `clearTimeout` 全部。

### ✅ E2（已修復，`a11e404`）. `src/screens/SettingScreen.js:76-93` — shimmer 動畫迴圈只在 unmount 時停止，載入完成後仍持續執行
...
**修復狀態**：effect 依賴陣列改為 `[isLoadingProfile, isLoadingSettings, shimmerAnim]`，兩者皆為 false 時提早 return，不再啟動迴圈；cleanup 邏輯不變，載入完成瞬間即會觸發 stop。

### E3（評估後跳過）. `src/hooks/useAppLoading.js:31-37` — 模組層級可變單例 `sharedSettingsPromise`（低優先地雷）
...
**跳過原因**：這是假設性的未來風險（審查時也承認「暫無實際問題」），目前只有單一呼叫點；在沒有真正的多實例場景前先做這個改動屬於過度設計，不符合專案「不要為假設性未來需求設計」的原則，故不動。

---

## F. 重複程式碼（boilerplate duplication，🟡 中優先）

### ✅ F1（已修復，`ddc47bb`）. `src/services/userService.js` — 網路錯誤判斷邏輯重複 7-8 次
...
**修復狀態**：新增 `UserService.isNetworkError(error)` static method（採用最完整的判斷條件），8 處呼叫點全部改用它。

### ✅ F2（已修復，`b075889`）. 使用者顯示名稱萃取邏輯重複 5+ 次
...
**修復狀態**：新增 `UserService.getUserDisplayName(user)`，`taskService.js` 5 處、`userService.js` 3 處全部改用它；同步更新 `taskService.test.js` 的 `UserService` mock。

### ✅ F3（已修復，`73bc3e3`）. `src/components/calendar/DatePickerOverlay.js` 與 `TimePickerOverlay.js` — 約 90% 程式碼重複
...
**修復狀態**：新增 `src/components/calendar/BottomSheetOverlay.js`，兩個檔案改為只傳入各自的 `<DateTimePicker>` 作為 children；外部呼叫端（`EditTaskModal`）props 介面不變。

### ✅ F4（已修復，`e6a24f5`）. `src/components/AdBanner.js:64-91` — `ADS_PAUSED` 為 true 期間仍執行完整使用者狀態查詢
...
**修復狀態**：effect 開頭加上 `ADS_PAUSED` 提早 return；新增 `cancelled` guard，unmount 後不再對已卸載元件呼叫 setState。
...
- ✅（已修復，`26aa70d`）`src/types/taskTypes.js:48-49`（`validateTaskFields`）— `Object.values(TASK_FIELDS)` 在 `forEach` 迴圈內每次重建 + O(n) `includes`；由 `taskService.createTaskObject` 每次建立/更新任務都會呼叫。已提到模組層級 `const VALID_TASK_FIELD_KEYS = new Set(Object.values(TASK_FIELDS))`，改用 `.has()`。
- ✅（已修復，`26aa70d`）`src/utils/mapUtils.js:10-14`（`isGoogleMapsUrl`）— 每次呼叫重建 3 個 `RegExp`，在非 memo 的 `MapPreview.js:32` 元件本體內呼叫，任務列表每次重繪都重建。已把 pattern 陣列移到模組層級常數 `GOOGLE_MAPS_URL_PATTERNS`。
- ✅（已修復，`695ab2c`）`ios/TaskCalWidget/TaskCalWidget.swift` 的 `DateFormatter`（見 D2）與 `UserDefaults(suiteName:)`（見 D2）未快取，性質相同，已併入 D 區處理。
...
**2026-07-11（獨立工作階段）** 完成 B2（`dffd6ea`）— `SettingScreen.js` 拆成 5 個 `memo` 化子元件（`ProfileCard`/`DeveloperToolsCard`/`GeneralSettingsCard`/`SupportLegalCard`/`AboutVersionCard`），主檔案從 2561 行降到約 1360 行，並將傳給子元件的 handler 全部改用 `useCallback`。已通過 jest 測試套件（23 個測試）、babel 語法解析、`expo export` Metro bundle 驗證，並經使用者在模擬器上手動 smoke test 各互動路徑確認無異常。
...
清單中的待辦項目已全數處理完畢（含跳過項目的評估）。B2 已在模擬器上手動驗證各互動路徑確認正常；其餘項目仍僅通過 jest 測試套件與編譯驗證，未做實機/模擬器 smoke test。
